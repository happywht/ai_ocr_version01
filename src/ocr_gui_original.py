...
class UniversalOCRGUI:
    ...
    def __init__(self):
        ...
        try:
            ...
            logging.warning(f"Excel导出功能不可用: {e}")
    ...
    def init_ocr_tool(self):
        ...
        try:
            ...
            if mode == 'drawing':
                ...
                logging.info("✅ 图纸OCR工具初始化成功")
            else:  # invoice
                self.ocr_tool = InvoiceOCRTool(use_ai=True)
                logging.info("✅ 发票OCR工具初始化成功")
        except Exception as e:
    ...
    def on_mode_change(self):
        ...
        logging.info(f"切换到{mode_text}模式")
        self.clear_results()
            ...
            if len(values) >= 4:
                ...
                if hasattr(self, 'file_paths') and self.file_paths:
                    ...
                    if 0 <= item_index < len(self.file_paths):
                        files.append(self.file_paths[item_index])
                    else:
                        logging.warning(f"无法找到文件路径，项目索引: {item_index}")
                else:
                    logging.warning(f"文件路径信息不完整，项目ID: {item}")
        ...
        try:
            for i, file_path in enumerate(files):
                try:
                    ...
                    # 处理文件（使用完整路径）
                    if mode == 'drawing':
                        result = self.ocr_tool.process_drawing(file_path)
                    else:
                    ...
                    # 提取OCR和AI信息
                    ocr_status = result.get('OCR状态', '未知')
                    ai_confidence = result.get('AI置信度', 0)
                    ...
                    logging.error(f"处理文件失败: {file_name}, 错误: {e}")
    ...
    def update_file_status(self, item_id, status, ocr_status="", ai_confidence=""):
        """更新文件处理状态"""
        try:
            item = self.file_tree.get_children()[item_id - 1]
    ...
    def display_results(self):
        ...
        for i, result in enumerate(self.current_results, 1):
            ...
            summary_text += f"📄 文件 {i}: {result.get('图片路径', '未知')}\n"
            summary_text += f"   状态: {result.get('OCR状态', '未知')}\n"
            summary_text += f"   AI置信度: {result.get('AI置信度', 0):.1%}\n"
            summary_text += f"   提取字段数: {len(result.get('提取字段', {}))}\n\n"
        ...
        for i, result in enumerate(self.current_results, 1):
            detail_text += f"{'='*60}\n"
            detail_text += f"📄 文件 {i}: {result.get('图片路径', '未知')}\n"
            detail_text += f"⏰ 处理时间: {result.get('处理时间', '未知')}\n"
            detail_text += f"🔍 解析方式: {result.get('解析方式', '未知')}\n"
            detail_text += f"📊 OCR状态: {result.get('OCR状态', '未知')}\n"
            detail_text += f"🤖 AI状态: {result.get('AI状态', '未知')}\n\n"
        ...
        try:
            ...
            if self.ocr_mode.get() == 'drawing':
                ttk.Button(button_frame, text="🎯 检测图签区域",
                          command=lambda: self.detect_signature_region(file_path, preview_window)).pack(side=tk.LEFT, padx=5)
    ...
    def detect_and_show_signature(self, file_path: str, parent_window):
        """检测并显示图签区域"""
        try:
            ...
            # 创建图签检测窗口
            signature_window = tk.Toplevel(parent_window)
            signature_window.title("图签区域检测结果")
            signature_window.geometry("900x700")
            signature_window.transient(parent_window)
                ...
                try:
                    with Image.open(file_path) as img:
                        ...
                        parent_window.after(0, lambda: self.display_signature_result(
                            img, signature_region, original_size, signature_window, file_path))

                except Exception as e:
                    parent_window.after(0, lambda: messagebox.showerror("检测失败", f"图签检测失败:\n{str(e)}"))
        ...
        try:
            ...
            def detect_thread():
                try:
                    ...
                    self.root.after(0, lambda: messagebox.showerror("检测失败", f"图签检测失败:\n{str(e)}"))
                finally:
        ...
        try:
            ...
            if signature_region:
                ...
                try:
                    ...
                    logging.error(f"裁剪预览显示失败: {e}")

            else:
                # 未检测到图签区域
                self.preview_canvas.create_text(15, 50, text="❌ 未检测到图签区域",
                                                   fill='gray', font=('Microsoft YaRefUi', 12),
                                                       anchor='w')

        except Exception as e:
            logging.error(f"显示图签结果失败: {e}")
            messagebox.showerror("显示错误", f"显示图签检测结果失败:\n{str(e)}")
